# https://brunch.co.kr/@mapthecity/2#comment
...
mergeUserDic(data.frame(c("현대상선, 대한해운, 흥아해운, 부산항, 광양항, 울산항, 인천항"),
                        "ncn"))
extractNoun(TEXTFILE)

SimplePos22(TEXTFILE)
# read in some stopwords:

library(tm)
stop_words <- stopwords("SMART") # 불용어 처리 

# pre-processing:
reviews <- gsub("'", "", TEXTFILE)  # remove apostrophes
reviews <- gsub("[[:punct:]]", " ", TEXTFILE)  # replace punctuation with space
reviews <- gsub("[[:cntrl:]]", " ", TEXTFILE)  # replace control characters with space
reviews <- gsub("^[[:space:]]+", "", TEXTFILE) # remove whitespace at beginning of documents
reviews <- gsub("[[:space:]]+$", "", TEXTFILE) # remove whitespace at end of documents
reviews <- tolower(TEXTFILE)  # force to lowercase

# tokenize on space and output as a list:
# doc.list <- strsplit(reviews, "[[:space:]]+")
doc.list <- strsplit(TEXTFILE, "[[:space:]]+")
